...
pub async fn get_streams_list(configuration: &configuration::Configuration, ) -> Result<models::StreamList, Error<GetStreamsListError>> {
...
pub async fn read_stream(configuration: &configuration::Configuration, stream_id: &str, send_content_events: Option<bool>, send_idea_drawer_feedback_events: Option<bool>, backfill_id: Option<&str>) -> Result<models::ReadStream200Response, Error<ReadStreamError>> {
    ...
    if let Some(ref param_value) = p_send_idea_drawer_feedback_events {
        req_builder = req_builder.query(&[("sendIdeaDrawerFeedbackEvents", &param_value.to_string())]);
    }
