...
pub async fn get_chatbot_rooms(configuration: &configuration::Configuration, chatbot_id: i32) -> Result<models::RoomList, Error<GetChatbotRoomsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_chatbot_id = chatbot_id;

    let uri_str = format!("{}/ib/v1/chatbot/{chatbot_id}/rooms", configuration.base_path, chatbot_id=p_chatbot_id);
...
pub async fn post_chatbot_message(configuration: &configuration::Configuration, chatbot_post_request: models::ChatbotPostRequest) -> Result<serde_json::Value, Error<PostChatbotMessageError>> {
    // add a prefix to parameters to efficiently prevent name collisions
